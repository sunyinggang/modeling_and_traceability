...
# from belief_network import GREEN, NORMAL
from graph_tool import Node, UndirectedNode, connect
from graph_tool import Graph, UndirectedGraph
from function_tool import get_args, named_base_type_factory
from function_tool import get_original_factors
...
class BBNNode(Node):
    ...
    # 重写__repr__方法，用于实现BBNNode对象的“自我描述”
    def __repr__(self):
        return '<BBNNode %s (%s)>' % (
            self.name,
            self.argspec)
...
class BBN(Graph):
    ...
    def q(self, kwds):
        ...
        tab = PrettyTable(['indicator name', '指标值', '发生概率'], sortby='indicator name')
        tab.align = 'l'
...
def build_bbn(*args, **kwds):
    ...
    # 判断args第一个元素是否是列表，若不是，则跳过此步骤
    if isinstance(args[0], list):
        args = args[0]

    for factor in args:
        #获取传入函数的参数的名称，factor是函数
    ...
    # original_factors存bbn_node，factor_nodes.values()是所有的bbn_node
    original_factors = get_original_factors(factor_nodes.values())
    # original_factors存bbn_node
...
def make_node_func(variable_name, conditions):
    ...
    node_func.__name__ = 'f_' + variable_name.encode('utf-8')
    return node_func
